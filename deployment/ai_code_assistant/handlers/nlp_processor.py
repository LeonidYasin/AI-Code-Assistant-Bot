...
class NLPProcessor:
    ...
    async def process_command(self, text: str, context: ContextTypes.DEFAULT_TYPE) -> Tuple[bool, str]:
        ...
        try:
            ...
            error_msg = (
                f"❌ Критическая ошибка при обработке запроса: {str(e)}\n\n"
                "Пожалуйста, проверьте логи для подробной информации. "
                "Попробуйте перезапустить бота или обратитесь в поддержку, если проблема не исчезнет."
            )
            logger.critical(f"Critical error in process_command: {e}", exc_info=True)
            await self._send_message(context, error_msg)
            return False, error_msg
